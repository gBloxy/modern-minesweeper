...
try:
    ...
    # avoid windows auto rescaling to make visible the entire window on low resolutions screens
...
themes = {
    ...
    1: {
        ...
        "flag": "flag2.png",
        "letters": {1: "I", 2: "II", 3: "III", 4: "IV", 5: "V", 6: "VI", 7: "VII", 8: "VI/nII"},
    },
    2: {
        # classic theme (0)
    },
    3: {"colors": {1: "blue4", 2: "green4", 3: "red3", 4: "darkblue", 5: "darkred", 6: "purple3", 7: "black", 8: "darkgray"}},
    4: {
        "colors": {1: "yellow", 2: "orange", 3: "orangered", 4: "red3", 5: "darkred", 6: "lightgray", 7: "darkgray", 8: "black"},
        "flag": "flag2.png",
    },
...
time_icon = pygame.transform.scale(pygame.image.load("asset//time_icon.png"), (20, 23)).convert_alpha()
...
themes_img = [pygame.image.load("asset//theme" + str(i) + ".png").convert() for i in range(1, 6)]
...
class DifficultyChooser:
    ...
    def __init__(self):
        ...
        self.triangle_points = [
            (self.rect.right - 10, self.rect.top + 11),
            (self.rect.right - 18, self.rect.top + 11),
            (self.rect.right - 14, self.rect.top + 17),
        ]
    ...
    def update(self):
        if self.rect.collidepoint(mouse_pos):
            ...
            if left_click:
                ...
                for i in range(5):
                    if pygame.Rect(
                        self.themes_rects[i].x + self.rect.left, self.themes_rects[i].y + self.rect.top, 120, 120
...
async def main():
    global mouse_pos, left_click, sound, sound_switch_timer, screen_shake, screen_shake_timer, scrolling
    while True:
        dt = clock.tick(30)
        ...
        if (
            keys[pygame.K_m] or (sett.active and left_click and sett.sound_rect.collidepoint(mouse_pos))
        ) and sound_switch_timer <= 0:
            sound = not sound
        ...
        for y, row in enumerate(Map):
            for x, tile in enumerate(row):
                ...
                if hovered and not ui_hovered and not (tile.revealed and (tile.type == 0 or tile.type == -1)):
                    pygame.draw.rect(
                        win, tile.hovered_color, (x * TILE_SIZE + offset[0], y * TILE_SIZE + offset[1], TILE_SIZE, TILE_SIZE)
                    )
                else:
                ...
                if tile.revealed and tile.type != 0:
                    if tile.type == -1:
                        ...
                        blit_center(
                            win, numbers[TILE_SIZE][tile.type], (tile.rect.centerx + offset[0], tile.rect.centery + offset[1] - 35)
                        )
        ...
        blit_center(
            win, menu_font.render(str(current_mines), True, COLOR), (WIN_SIZE[0] - (80 if dc.current_diff != "easy" else 62), 17)
        )
